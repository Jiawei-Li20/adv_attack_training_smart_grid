...
# maxcapacity=len(plant_cost_curve)-1
...
#     print(plant_cost_curve[(l*np.sqrt(var)+ mean).astype(np.int64)+1])
...
def cost_loss(y_true,y_pred):
    #print(y_pred.shape)
    # to do: how to judge y_pred > maxcapacity,
    # to do: assume the marginal cost for those exceed one is the maximum marginal cost, marginal_cost[-1]
...
parser.add_argument('-trainaim', type=str, help='pred or cost')
# parser.add_argument('attack_aim', type=str, help='pred or cost')
args = parser.parse_args()

adv_example = args.trainaim#"pred" or "cost"
adv_attack = args.trainaim
print(adv_example)
data_file = "adv_" + args.trainaim + "_attack_" + args.trainaim+"80"
attack_plain_model = "test_with_plain_model_"#  or ""
# epochs = 1
# attack_times = 1
...
print("Scaler mean =", scaler.mean_,mean)
print("Scaler var =", scaler.var_,var)
...
y_real = y_test
if adv_example == "cost":
    plt.plot(y_result[:, 0]*np.sqrt(var)+mean, 'r')
    plt.plot(y_real[:, 0]*np.sqrt(var)+mean, 'b')
...
for outer_loop in range(11):
    model_path = 'rnn_substitute.h5'
    model_test = rnn_model(seq_length=seq_length, input_dim=feature_dim, output_dim=forecast_horizon)
    if adv_example == "cost":
        model_test.compile(loss=cost_loss, optimizer='adam')
    else:
    ...
    mae[outer_loop] = calculate_mae(attacked_data, df['actual'][start:end]*loadratio)
    # import ipdb;ipdb.set_trace()
...
for outer_loop in range(1, 11):
    ...
    with tf.Session() as sess:
        ...
        if attack_plain_model == "":
            model.load_weights(data_file +f'rnn_advtrained_eps_{outer_loop}.h5')
        else:
        ...
        model_advtrain.load_weights(data_file+f'rnn_advtrained_eps_{outer_loop}.h5')# shall we add it here?
        ...
        plt.ylabel('Load (MW)')
        plt.legend()
        plt.savefig(data_file + '/'+attack_plain_model+'load_attack_bound%.2f_result.png' % eps, dpi=150)
        # plt.show()

        with open(data_file + '/'+attack_plain_model+'load_attack_bound%.2f_result.csv' % (eps), 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerows(adversarial_data)
        with open(data_file + '/'+attack_plain_model+'pred_data%.2f.csv' % (eps), 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerows(pred_data)
        with open(data_file + '/'+attack_plain_model+'original_data%.2f.csv' % (eps), 'w', newline='') as f:
            writer = csv.writer(f)
        ...
        plt.savefig(data_file + '/'+attack_plain_model+'load_attack_to_advtrain_noise%.2f_temp.png' % eps, dpi=150)
