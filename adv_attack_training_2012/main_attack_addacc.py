...
y_train_acc = np.load(f"adv_results/{acc_rate}mc4.0wind_acceptable_load.npy")/1.4
# y_train_acc = np.concatenate((np.zeros(6000),y_train_acc))
# print(y_train_acc[0:10]/1.4,df['actual'][0:10])
df_y = pd.DataFrame(y_train_acc, columns=['acc_value'], index = df.index)
print("Datasize shape:", df.shape, df_y.shape)
...
x_train_acc = np.copy(X_test)
y_train_acc = np.copy(df_scaled[6000:]['acc_value'].copy())
print(x_train_acc.shape, y_train_acc.shape)
x_train_acc, y_train_acc  = reorganize2(x_train_acc, y_train_acc, seq_length, forecast_horizon, forecast_time)
...
x_train_acc = np.array(x_train_acc, dtype=float) #include 'actual'
y_train_acc = np.array(y_train_acc, dtype=float).reshape(-1, forecast_horizon)
...
x_newtrain = np.concatenate((x_train, x_train_acc), axis=0)
y_newtrain = np.concatenate((y_train, y_train_acc), axis=0)

model.fit(x=x_newtrain, y=y_newtrain, batch_size=batch_size, epochs=epochs, shuffle=True)
model.save_weights('results_2022/%.2frnn_cleanfivesteps.h5'%acc_rate)
model.save_weights('results_2022/%.2frnn_substitute.h5'%acc_rate)

y_result = model.predict(x_test)
y_real = y_test
plt.plot(y_result[:300, 0], 'r')
plt.plot(y_real[:300, 0], 'b')
plt.savefig('results_2022/%.2ftraining_result.png'%acc_rate, dpi=150)
# plt.show()
...
with sess.as_default():
    # for outer_loop in range(1, 21):
    for outer_loop in [int(acc_rate*100)]:
        X_new = []
        grad_new = []
        ...
        model.load_weights('results_2022/%.2frnn_substitute.h5'%acc_rate)
        ...
        for q in range(opt_length - seq_length):
            ...
            for it in range(30):
                ...
                # if q==0:
                #     print(X_input[:,:,0], grad_sign[:,:,0])
                signed_grad[:, :, 0] = bound*40*grad_sign[:, :, 0]
        ...
        model.load_weights('results_2022/%.2frnn_cleanfivesteps.h5'%acc_rate)
        ...
        print("Adversarial MAPE is: %f, with bound %f" % (adv_mae_val, eps))
        mae_val = calculate_mae(pred_data[:, 0], original_data[:, 0])
        print("Prediction MAPE is: %f, with bound %f" % (mae_val, eps))
        ...
        plt.ylabel('Load (MW)')
        plt.legend()
        plt.savefig('results_2022/%.2fload_attack_bound%.2f_mae%.4f_result.png' % (acc_rate, eps, adv_mae_val), dpi=150)
        # plt.show()

        with open('results_2022/%.2fload_attack_bound%.2f_result.csv' % (acc_rate, eps), 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerows(adversarial_data)
        with open('results_2022/%.2fpred_data.csv'% (acc_rate), 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerows(pred_data)
        with open('results_2022/%.2foriginal_data.csv'% (acc_rate), 'w', newline='') as f:
            writer = csv.writer(f)
        ...
        plt.savefig('results_2022/%.2fload_attack_noise%.2f_temp.png' % (acc_rate, eps), dpi=150)
        # plt.show()

        # with open('results/load_attack_noise%.2f_bb3.csv' %eps, 'w', newline='') as f:
        #     writer = csv.writer(f)
        #     writer.writerows(adversarial_data)
...
# # ==============================
# print('\n' * 2 + '=' * 30 + '\nGradient estimation attack begins.\n' + '=' * 30)
...
#     plt.ylabel('Load (MW)')
#     plt.legend()
#     plt.savefig('results/load_grad_noise%.2f_bb4.png' % eps, dpi=150)
#     # plt.show()

#     with open('results/load_grad_noise%.2f_bb4.csv' % eps, 'w', newline='') as f:
#         writer = csv.writer(f)
#         writer.writerows(adversarial_data)
...
#     plt.legend()
#     plt.ylabel('Temperature (F)')
#     plt.savefig('results/load_grad_noise%.2f_bb3.png' % eps, dpi=150)
#     # plt.show()

#     with open('results/load_grad_noise%.2f_bb3.csv' % eps, 'w', newline='') as f:
#         writer = csv.writer(f)
#         writer.writerows(adversarial_data)

with open('results/%.2fload_original.csv'%acc_rate, 'w') as f:
    writer = csv.writer(f)
    writer.writerows(original_data)