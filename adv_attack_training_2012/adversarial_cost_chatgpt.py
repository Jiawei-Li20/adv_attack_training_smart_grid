...
#     res += tf.where(tf.math.greater_equal(original_y,lb), base + mc[-1] * (original_y-offset), tf.zeros_like(original_y))
#     # print(res)
#     return res
...
model = rnn_piecewise_model(seq_length=seq_length, input_dim=feature_dim, output_dim=forecast_horizon)
predictions = model(x)
...
for outer_loop in range(1, 11):
    ...
    model_advtrain = rnn_piecewise_model(seq_length=seq_length, input_dim=feature_dim, output_dim=forecast_horizon)
    predictions = model(x)
    sgd = SGD(lr=0.0001, decay=1e-6, momentum=0.9, nesterov=True)
    model_advtrain.compile(loss='mse', optimizer='adam')
    ...
    with sess.as_default():
        ...
        with open('adv_results/pred_data%.2f.csv' % (eps), 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerows(pred_data)
        with open('adv_results/original_data%.2f.csv' % (eps), 'w', newline='') as f:
            writer = csv.writer(f)
