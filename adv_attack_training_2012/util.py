...
def reorganize2(X_train, Y_train, seq_length, forecast_horizon, forecast_time):
    ...
    for i in range(len(X_train) - seq_length-forecast_horizon-forecast_time):
        ...
        if i==0:
            print(y_new.shape)
        x_new = np.concatenate((y_new, x_new), axis=1)
        if i==0:
            print(x_new.shape)
        x_data.append(x_new)
        ...
        if i==0:
            ...
            print(i+forecast_time-1+seq_length,i+forecast_horizon+forecast_time+seq_length-1)
            print(y_new.shape)
        y_data.append(y_new)
...
def calculate_mae(pred, orig):
    ...
    for i in range(len(pred)):
        mae+=np.abs(pred[i]-orig[i])/np.abs(orig[i])
