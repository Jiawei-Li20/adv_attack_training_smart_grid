...
def run(bound):
    ...
    #windratio=6.5
        ...
        return emi_cost_hatB_trivialfirst
...
for bound in bound_list:
    osc_th = run(bound/100.0)
    OSC_th_list.append(osc_th)
result  = pd.DataFrame({"theory": OSC_th_list})
    # opt_list.append(opt)
    # no_list.append(no)
...
result.to_csv(f"result_theory_{adv_attack}_{prepath}_{mixed}_{args.net}{attacked}.csv")
